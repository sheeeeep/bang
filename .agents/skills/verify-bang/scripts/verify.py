...
def doctor():
    ...
    for relative in (
        "bang.py",
        "test_terminal.py",
        "bang_templates/AGENTS.common.md",
        "bang_templates/AGENTS.js.md",
        "bang_skills/install/SKILL.md",
    ):
        assert (ROOT / relative).is_file(), relative
    result = subprocess.run(
        [sys.executable, "-B", str(ROOT / "bang.py"), "--help"],
        capture_output=True,
        text=True,
        timeout=10,
        check=True,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="bang-verify-") as temporary:
            ...
            try:
                ...
                report["passed"] = True
                report["final_state"] = (
                    "链接已移除；个人库内容未变；exclude 恢复；未创建 .gitignore"
                )
            finally:
                for index, session in enumerate(sessions):
                    session.close()
                    (evidence / f"terminal-{index}.txt").write_text(
                        session.text(), encoding="utf-8"
                    )
    finally:
